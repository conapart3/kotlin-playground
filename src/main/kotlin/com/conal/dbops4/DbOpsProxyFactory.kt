...
class DbOpsProxyFactory {
    ...
    fun <T : DbOps> proxy(delegate: T, targetInterfaces: List<Class<T>>): DbOps {
//        require(targetInterface.isInterface) { "Interface is expected instead of $targetInterface" }
        val handler = DbOpsProxyHandler(delegate, targetInterfaces)
        @Suppress("UNCHECKED_CAST")
        // we want to assign the proxy to fields of type "MyIntermediateDbOps" and "MyIntermediateDbOps".
        return Proxy.newProxyInstance(delegate::class.java.classLoader, targetInterfaces.toTypedArray(), handler) as DbOps
    }

    private class DbOpsProxyHandler(
        val delegate: Any,
        private val interfaces: List<Class<out DbOps>>,
    ) : InvocationHandler {
        ...
        //        @Suspendable
        override fun invoke(proxy: Any, method: Method, args: Array<out Any?>?): List<*> {
            ...
            // test see what happens if it just sends request to the delegate...
//            return method.invoke(delegate, *arguments)// works
