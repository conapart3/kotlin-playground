...
internal class CordaFutureImpl<V>(private val impl: CompletableFuture<V> = CompletableFuture()) : Future<V> by impl, OpenFuture<V> {
    ...
    override fun getOrThrow(timeout: Duration): V = (this as Future<V>).getOrThrow(timeout)

    override fun getOrThrow(): V = (this as Future<V>).getOrThrow()

    override fun then(callback: (CordaFuture<V>) -> Unit): Unit = thenImpl(defaultLog, callback)

    override fun then(callback: Consumer<CordaFuture<V>>): Unit = then { callback.accept(it) }

    /** For testing only. */
    @Suppress("TooGenericExceptionCaught")
    internal fun thenImpl(log: Logger, callback: (CordaFuture<V>) -> Unit) {
