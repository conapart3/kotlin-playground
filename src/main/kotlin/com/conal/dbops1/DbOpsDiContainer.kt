        ...
        flow::class.getFieldsForDbOpsInjection()
            .forEach { field ->
                ...
                if(field.get(flow) == null){
                    val proxy = getProxy(field)
                    field.set(flow, uncheckedCast(proxy))
                }
            }
    }

    private fun getProxy(field: Field): DbOps {
//        if(type.isInterface){
//            println("DbOps DI injection $field is interface but should be implementation.")
...
//        }
        val annotation = field.annotations.find { it is DbOpsInject } as DbOpsInject
        val implWithProxy = dbOpsMap[annotation.impl.java]
//        val implWithProxy = dbOpsMap[field.type]
        if (implWithProxy == null) {
            // probably don't want to throw an exception just skip the dbOps that couldn't be injected?
            println("Attempted to inject dbOps dependency but $field is not registered.")
            throw DbOpsException("DbOps DI injection has no implementation of $field.")
        }
        return implWithProxy.proxy
    }
}
